...
server.tool("get_trending", "Get GitHub trending repositories by time period", {
    ...
    const summary = repos.slice(0, 10).map((r) => ({
...
server.tool("search_repos", "Search GitHub repositories", {
    query: z.string().describe("Search keyword"),
}, async ({ query }) => {
    const data = await fetchAPI(`/search?q=${encodeURIComponent(query)}`);
    const repos = (data.repos || data || []).slice(0, 10).map((r) => ({
...
server.tool("compare_repos", "Compare two GitHub repositories side by side", {
...
server.tool("get_chaoss_health", "Evaluate open source project health using CHAOSS metrics with Goal-Question-Metric framework", {
...
server.tool("get_summary", "Generate an AI-style trend analysis report for GitHub trending repos", {
...
server.tool("get_chaoss_metrics_info", "Get the CHAOSS metrics framework definition used by TrendPulse", {}, async () => {
...
server.tool("get_repo_detail", "Get detailed information about a GitHub repository", {
