...
const DailyPage = () => {
  ...
  const fetchDailyTrending = useCallback(async (isManual = false) => {
    if (isManual) {
      setRefreshing(true);
    } else {
      setLoading(true);
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h2 className="page-title" style={{ marginBottom: 0 }}>每日热点仓库</h2>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <Button
            ...
            size="small"
            loading={refreshing}
            onClick={() => fetchDailyTrending(true)}
      ...
      {loading ? (
        ...
        <Alert title={error} type="error" showIcon action={
          <Button size="small" onClick={() => fetchDailyTrending(true)}>重试</Button>
        } />
      ) : repos.length === 0 ? (
        <Alert title="暂无数据" type="info" showIcon />
      ) : (
        <RepositoryList repos={repos} period="daily" />
      )}
    </div>
  );
};
